...
def build_report(panel=None, db_path=None) -> str:
    if panel is None:
    ...
    ic = run_ic(HORIZONS, panel=panel)
    rank_ic = run_rank_ic(HORIZONS, panel=panel)
    decay = run_decay(horizons=HORIZONS, panel=panel)
...
def main():
    ...
    md = build_report(db_path=args.db_path)
